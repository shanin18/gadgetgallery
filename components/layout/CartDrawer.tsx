      ...
      <aside className={`fixed right-0 top-0 z-[80] h-full w-full max-w-md border-l bg-background shadow-soft transition-transform duration-300 ${open ? "translate-x-0" : "translate-x-full"}`}>
        ...
        <div className="flex h-[calc(100%-4rem)] flex-col">
          <div className="flex-1 space-y-4 overflow-auto p-5">
            ...
            {items.map((item) => (
              <div key={item.id} className="grid grid-cols-[72px_1fr] gap-3 rounded-lg border bg-card p-3">
                ...
                <div>
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => removeItem(item.id)} className="rounded p-1 text-muted-foreground hover:bg-muted hover:text-destructive" aria-label="Remove item">
                      <Trash2 size={16} />
                    </button>
                  </div>
                  {item.selectedOptions.length ? <p className="mt-1 text-xs font-semibold text-muted-foreground">{item.selectedOptions.map((option) => `${option.name}: ${option.value}`).join(", ")}</p> : null}
                  <p className="mt-1 text-sm font-bold">{formatBDT(item.unitPrice)}</p>
                  <div className="mt-3 inline-flex items-center rounded-md border">
                    <button className="p-2" onClick={() => setQuantity(item.id, item.quantity - 1)} aria-label="Decrease quantity"><Minus size={14} /></button>
                    <span className="w-8 text-center text-sm font-semibold">{item.quantity}</span>
                    <button className="p-2" onClick={() => setQuantity(item.id, item.quantity + 1)} aria-label="Increase quantity"><Plus size={14} /></button>
                  </div>
                </div>
              </div>
            ))}
